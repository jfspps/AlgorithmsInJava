...
public class LinkedList<T> {
    ...
    /**
     * getNode() traverses the list and returns the first instance of LinkedList with given data, hence O(n)
     * can also be used to check if data is present
    */
    ...
    /**
     * Determines is a given node reference is valid for the list; time complexity is O(n)
     */
    ...
    /**
     * Adds to an array of references to LinkedLists, from index i (inc.), which have the value data;
     * searches the entire list, so is always O(n)
     */
    ...
    /**
     * Appends an array of nodes, preserving the order, to the tail of the current linked list; time complexity is O(n)
     * */
     ...
     * returns a reference to the second node (which can be null) if the head is deleted, and returns head otherwise;
     */
     ...
     * checks if k is part of list and cycles through nodes after k, so time complexity is at worst O(n)
     */
